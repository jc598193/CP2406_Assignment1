...
//            if (this.position[0] + this.getSpeed() < this.currentRoad.end_location[0]) {
//                this.position[0] = this.position[0] + this.speed;
//            }
//            // car in the end
//            else if (this.position[0] + this.speed == this.currentRoad.end_location[0]) {
//                this.position[0] = this.position[0] + this.speed;
//                // road still has light and it is red
//                if (!this.currentRoad.getLightsOnRoad().isEmpty() && this.currentRoad.getLightsOnRoad().get(0).getLocation()[0] > this.position[0] && this.currentRoad.getLightsOnRoad().get(0).getState().equals("red")) {
//                    this.stop();
...
//                } else {
//                    if (!this.currentRoad.getConnectedRoad().isEmpty()){
//                        int pass_distance = this.currentRoad.end_location[0] - this.position[0];
...
//            this.speed = this.currentRoad.getSpeedLimit();
//            // car is not in the end:
//            if (this.position[1] + this.speed < this.currentRoad.end_location[1]) {
//                this.position[1] = this.position[1] + this.speed;
//            }
//            // car in the end
//            else if (this.position[1] + this.speed == this.currentRoad.end_location[1]) {
//                this.position[1] = this.position[1] + this.speed;
//                // road still has light and it is red
//                if (!this.currentRoad.getLightsOnRoad().isEmpty() && this.currentRoad.getLightsOnRoad().get(0).getLocation()[1] > this.position[1] && this.currentRoad.getLightsOnRoad().get(0).getState().equals("red")) {
//                    this.stop();
//                    // road still has light and it is green
//                    // or road does not have light
//                    // or light is passed
...
//    }
    ...
    public void move() {
        if (this.currentRoad.getVector().equals("horizontal")){
            ...
            if (!this.currentRoad.getLightsOnRoad().isEmpty() && this.position == this.currentRoad.getLightsOnRoad().get(0).getLocation() && this.currentRoad.getLightsOnRoad().get(0).getState().equals("red")) {
                this.stop();
            } else {
                this.speed = this.currentRoad.getSpeedLimit();
                if (this.currentRoad.end_location == this.getPosition() && !this.currentRoad.getConnectedRoad().isEmpty()) {
                    this.currentRoad.getCarsOnRoad().remove(this);
                    this.currentRoad = this.currentRoad.getConnectedRoad().get(0);
                    this.currentRoad.getCarsOnRoad().add(this);
                    this.position = this.currentRoad.start_location;
                } else if (this.currentRoad.end_location[0] > this.getPosition()[0]) {
                    this.position[0] = (this.position[0] + this.speed);
                } else {
                    this.stop();
                }
            ...
            if (!this.currentRoad.getLightsOnRoad().isEmpty() && this.position == this.currentRoad.getLightsOnRoad().get(0).getLocation() && this.currentRoad.getLightsOnRoad().get(0).getState().equals("red")) {
                ...
                if (this.currentRoad.end_location == this.getPosition() && !this.currentRoad.getConnectedRoad().isEmpty()) {
                    ...
                    this.currentRoad.getCarsOnRoad().add(this);
                    this.position = this.currentRoad.start_location;
                } else if (this.currentRoad.end_location[1] > this.getPosition()[1]) {
                    this.position[1] = (this.position[1] + this.speed);
                } else {
                    this.stop();
                }
    ...
    public void printCarStatus() {
        System.out.printf("%s going:%dm/s on %s at position:%s%n", this.getId(), this.getSpeed(), this.getCurrentRoad().
                getName(), Arrays.toString(this.position));
    }
